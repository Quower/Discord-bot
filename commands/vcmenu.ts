...
export default {
  ...
  init: async (client: Client) => {
    ...
    client.on("messageCreate", async (msg) => {
      ...
      if (menuMessage) {
        if (
          channel === menuMessage.channel &&
          guild! === menuMessage.guild &&
          member!.id === "424279456031703041"
        ) {
          if (addingMembers === true) {
            msg.mentions.members!.forEach((mentions) => {
              if (
                !VcMenuMemberList.includes(mentions) &&
                mentions.id !== "424279456031703041"
              ) {
                VcMenuMemberList[VcMenuMemberList.length] = mentions;
              }
            });
            menuMessage.edit({
              components: [Update_message_row()],
            ...
            msg.mentions.members!.forEach((mentions) => {
              VcMenuMemberList = VcMenuMemberList.filter(function (
                value,
                index,
                arr
              ) {
      ...
      if (newState.channel === MenuVC) {
        if (VcMenu === true) {
          if (VcMenuwhitelist === true) {
            ...
            if (
              VcMenuMemberList.includes(member) &&
              member!.id !== "424279456031703041"
            ) {
              member?.voice.disconnect();
            } else {
  ...
  callback: async ({ channel, interaction }) => {
    ...
    if (menuMessage instanceof Message) {
...
function Update_message_embed() {
  ...
  if (VcMenu === true) {
    ...
    embed.addFields({ name: "Disabled", value: "Disabled", inline: true });
  }
  return embed;
}
